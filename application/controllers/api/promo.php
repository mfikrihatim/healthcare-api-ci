...
class promo extends RestController
{
    ...
    public function index_get()
    {
        ...
        if (isset($_GET['id_promo'])) {
            $query = $this->MSudi->GetDataWhere('promo', 'id_promo', $_GET['id_promo'])->result();
            $this->response([
                'status' => 200,
                'message' => 'success',
                'data' => $query
            ...
            $query = $this->MSudi->GetData('promo');
            $this->response([
                'status' => 200,
                'message' => 'success',
                'data' => $query
            ...
            if (isset($header['Authorization'])) {
                if ($header['Authorization'] == $this->token) {
                    ...
                    $insert = array(
                        ...
                        'exp_promo' => $this->input->post('exp_promo'),
                        'platform_promo' => $this->input->post('platform_promo'),
                        'ket_promo' => $this->input->post('ket_promo'),
                        'foto_promo' => $data['filenames'],
                        'hyperlink_promo' => $this->input->post('hyperlink_promo'),
                    );

                    $query = $this->MSudi->AddData('promo', $insert);
                    $this->response([
                        'status' => 200,
                        'message' => 'success',
                        'data' => $query
    ...
    public function update()
    {
        ...
        if (isset($header['Authorization'])) {
            if ($header['Authorization'] == $this->token) {
                ...
                $query = $this->MSudi->UpdateData('promo', 'id_promo', $update['id_promo'], $update);
                $this->response([
                    'status' => 200,
                    'message' => 'success',
                    'data' => $query
    ...
    public function index_delete()
    {
        ...
        if (isset($header['Authorization'])) {
            if ($header['Authorization'] == $this->token) {
                ...
                $query = $this->MSudi->DeleteData('promo', 'id_promo', $delete['id_promo'], $delete);
                $this->response([
                    'status' => 200,
                    'message' => 'success',
                    'data' => $query
