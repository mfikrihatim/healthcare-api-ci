...
class faq extends RestController
{
    ...
    public function index_get()
    {
        ...
        if (isset($_GET['id_faq'])) {
            $query = $this->MSudi->GetDataWhere('faq', 'id_faq', $_GET['id_faq'])->result();
            $this->response([
                'status' => 200,
                'message' => 'success',
                'data' => $query
        ...
        if (isset($_POST['id_faq'])) {
            ...
            if (isset($header['Authorization'])) {
                if ($header['Authorization'] == $this->token) {
                    ...
                    $query = $this->MSudi->AddData('faq', $insert);
                    $this->response([
                        'status' => 200,
                        'message' => 'success',
                        'data' => $query
    ...
    public function update()
    {
        ...
        if (isset($header['Authorization'])) {
            if ($header['Authorization'] == $this->token) {
                ...
                $query = $this->MSudi->UpdateData('faq', 'id_faq', $update['id_faq'], $update);
                $this->response([
                    'status' => 200,
                    'message' => 'success',
                    'data' => $query
    ...
    public function index_delete()
    {
        ...
        if (isset($header['Authorization'])) {
            if ($header['Authorization'] == $this->token) {
                ...
                $query = $this->MSudi->DeleteData('faq', 'id_faq', $delete['id_faq'], $delete);
                $this->response([
                    'status' => 200,
                    'message' => 'success',
                    'data' => $query
